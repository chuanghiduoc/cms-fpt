...
export default function DepartmentEditModal({
  ...
  const [formData, setFormData] = useState<DepartmentUpdateInput>({
    name: '',
    description: null
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Không thể cập nhật phòng ban');
      }
      ...
      setError(err instanceof Error ? err.message : 'Đã xảy ra lỗi khi cập nhật phòng ban');
    } finally {
      setIsLoading(false);
    }
  };
