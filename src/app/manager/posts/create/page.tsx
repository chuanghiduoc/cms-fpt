...
export default function CreatePostPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, isDraft: boolean = false) => {
    ...
    try {
      // Không cần tải lên file nữa vì đã tải lên trước đó
      // Sử dụng trực tiếp URL
      const coverImageUrl = coverImage?.url || '';
      
      // Nếu bài viết công khai thì cần duyệt, còn nội bộ thì không cần duyệt
      const needsApproval = isPublic;
      const status = isDraft ? 'PENDING' : (needsApproval ? 'PENDING' : 'APPROVED');
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Đã xảy ra lỗi khi tạo bài viết');
      }

      // Không cần sử dụng dữ liệu trả về từ response
      await response.json();
      
      const successMessage = isDraft 
        ? 'Bài viết đã được lưu nháp thành công' 
        : (needsApproval 
            ? 'Bài viết công khai đã được gửi và đang chờ duyệt' 
            : 'Bài viết nội bộ đã được đăng thành công');
      
      toast.success(successMessage);
      router.push('/manager/posts');
    } catch (error) {
      console.error('Lỗi khi tạo bài viết:', error);
      toast.error(error instanceof Error ? error.message : 'Đã xảy ra lỗi khi tạo bài viết');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={loading}
              >
                <FiEye className="mr-2" /> Xem trước
              </button>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="flex">
                  ...
                  <button
                    ...
                    className="ml-2 mt-1 inline-flex items-center px-3 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 cursor-pointer"
                  >
                    Thêm
                  </button>
                </div>
