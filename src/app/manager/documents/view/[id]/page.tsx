...
import { FiFileText, FiDownload, FiEdit2, FiArrowLeft, FiCalendar, FiUser, FiFolder } from 'react-icons/fi';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {(session?.user?.role === 'ADMIN' || 
          (session?.user?.role === 'DEPARTMENT_HEAD' && document.department?.id === session?.user?.department)) && (
          <Link 
            href={`/manager/documents/edit/${document.id}`}
            className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-md flex items-center gap-2 transition duration-200"
          >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-start">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      {document.isPublic ? 'Đã duyệt' : 'Chưa duyệt'}
                    </span>
                  </div>
